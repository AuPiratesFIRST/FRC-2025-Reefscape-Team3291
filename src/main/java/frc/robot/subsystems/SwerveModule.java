...
public class SwerveModule {
    ...
    private final SimpleMotorFeedforward feedForward = new SimpleMotorFeedforward(Swerve.ffkS, Swerve.ffkV,
            Swerve.ffkA);
    ...
    public SwerveModule(int moduleNumber, SwerveModuleConstants moduleConstants) {
        ...
        // This is the offset of the wheel in relation to the 0 position of the
        // CANCoder.
        ...
        this.driveMotor = new SparkMax(moduleConstants.driveMotorId, MotorType.kBrushless);
        ...
        this.angleMotor = new SparkMax(moduleConstants.angleMotorId, MotorType.kBrushless);
    ...
    /*
     * 
     * This is intended to set the default strategy for each angle encoder
     * (CANCoder)
     */
     ...
     * configDriveMotor()
     * 
     * This is intended to set default values for this modules drive motor
     * (SparkMAX)
     */
    private void configDriveMotor() {
        ...
        CANSparkMaxUtil.setCANSparkMaxBusUsage(driveMotor, Usage.kAll, driveMotor.isFollower());// not sure if boolean
                                                                                                // value is right
        ...
        // Sets the voltage compensation setting for all modes on the SPARK and enables
    ...
    private void configAngleMotor() {
        ...
        CANSparkMaxUtil.setCANSparkMaxBusUsage(angleMotor, Usage.kPositionOnly, angleMotor.isFollower());// not sure if
                                                                                                         // boolean
                                                                                                         // value is
                                                                                                         // right
    ...
    private boolean setAngle(SwerveModuleState desiredState) {
        ...
        // Get the current state of the swerve module (i.e. What direction is it
        // facing?)
        ...
        // -190 is equivalent to 170 so the below ternaries convert numbers less than
        // -180
        // or greater than 180 into [-180,180]
        currentDegrees = currentDegrees < -180 ? currentDegrees + 360 : currentDegrees;
        currentDegrees = currentDegrees > 180 ? currentDegrees - 360 : currentDegrees;

        // Next figure out the what direction the service module should be pointing. We
        // mode the number
        // (% 360) to make sure the number is between [-360,360]
        double desiredDegrees = desiredState.angle.getDegrees() % 360;
        // The following logic calculates the the different between the desired angle of
        // the
        ...
        // In theory, we could turn the swerve to this "diff" angle and everything would
        // work but its
        // possible to be more efficient. For example, if the operator pushes the
        // joystick forward,
        // then pulls it full back, the code above will turn the swerve module 180
        // degrees when
        ...
        // If we are a degree or less off, the turning motor will not do anything, else,
        // calculate the pid
        // values based off the diff and 0.
        double turningMotorValue = Math.abs(diff) < 1 ? 0 : anglePid.calculate(diff, 0);
        // Motors can only take a value in the range [-1,1] but the PID may output a
        // value outside
        // this range, thus we "clamp" the output of the PID such that -1.5 would just
        // be -1.
    ...
    private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop, boolean invertDriveMotor) {
        ...
        // driveValue = feedForward.calculate(desiredState.speedMetersPerSecond/2) +
        // drivePid.calculate(driveEncoder.getVelocity(),
        // desiredState.speedMetersPerSecond/2);
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                (this.driveEncoder.getPosition()), // * (Swerve.wheelCircumference / (Swerve.driveGearRatio * 42)//
                                                   // Actually forgot where I got this from lmao
                this.getCanCoder());
    }
